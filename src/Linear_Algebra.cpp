...
// 1. Vector functions.
	...
	Vector Vector::operator+(Vector v)
	{
		...
		{
			std::vector<double> new_components(dimension);
			for(unsigned int i = 0; i < dimension; i++) new_components[i] = components[i] + v[i];
			return Vector(new_components);
		}
	...
	{
		std::vector<double> new_components(dimension);
		for(unsigned int i = 0; i < dimension; i++) new_components[i] = components[i] * s;
		return Vector(new_components);
	}
	...
	{
		for(unsigned int i = 0; i < dimension; i++) components[i] += v[i];
		return *this;
	}
	...
	Vector operator*(double s,const Vector &v)
	{
		...
		for(unsigned int i = 0; i < v.Size(); i++) new_components[i] = v[i] * s;
		return Vector(new_components);
	}
...
//Binary operations
	...
	Matrix Matrix::Product(const Matrix &M) const
	{
		...
		else
		{
			...
			for(unsigned int i = 0; i < result.Rows(); i++)
			{
				...
				{
					for(unsigned int k = 0; k < columns; k++) result[i][j] += components[i][k] * M[k][j];
				}
			}
			return result;
		}
	...
	Vector Matrix::Product(const Vector &v_rhs) const
	{
		...
		else
		{
			...
			for(unsigned int i = 0; i < rows; i++)
			{
				for(unsigned int j = 0; j < columns; j++) result_components[i] += components[i][j]*v_rhs[j];
			}
...
//Matrix operations
	...
	Matrix Matrix::Inverse() const
	{
		...
		else
		{
			...
			for(unsigned int i = 0; i < N; i++) A.Delete_Column(0);
			return A;
		}
	}
	...
	double Matrix::Trace() const
	{
		...
		{
			double tr = 0.0;
			for(unsigned int i = 0; i<rows; i++) tr += components[i][i];
			return tr;
		}
	}
